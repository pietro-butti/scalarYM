...
// #endif
    ...
    for (int thread=0; thread<maxthread; thread++) {
      ...
      nt=nx+1;
#if Ncol==2
      sprintf(infilename,"%d_medium_knl_input_thread_%d.dat",rowindex,thread);
#endif
#if Ncol==3
      sprintf(infilename,"%d_large_knl_input_thread_%d.dat",rowindex,thread);
#endif
